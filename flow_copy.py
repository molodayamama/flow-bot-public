...
MESSAGES = {
    ...
    "referral_screen": (
        "🤝 <b>Приглашай друзей — получай кредиты</b>\n\n"
        "Твоя ссылка:\n{link}\n\n"
        "Приглашено: <b>{invited}</b> · заработано: <b>{earned} кр.</b>\n\n"
        "Как это работает: друг переходит по ссылке и пополняет баланс —\n"
        "• любая первая оплата → <b>+{t1} кр.</b>\n"
        "• от среднего пакета → <b>+{t2} кр.</b>\n"
        "• от большого пакета → <b>+{t3} кр.</b>\n"
        "• дальше — <b>10%</b> от каждого его пополнения."
    ),
    "invite_share_text": (
        "Попробуй этого бота — рисует картинки и снимает видео по тексту 🎨🎬 "
        "Новым пользователям — бесплатные кредиты на старте!"
    ),
    "referral_reward_got": (
