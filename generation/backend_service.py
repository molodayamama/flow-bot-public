...
async def generate_video_ingredients(deps: Any, req: dict) -> dict:
    """Run 1-4 photos + prompt reference-to-video and return MP4 as base64."""
    prompt = str(req.get("prompt") or "").strip()
    if len(prompt) < 3:
        return {"error": "empty prompt"}
    encoded_images = req.get("images_b64")
    if not isinstance(encoded_images, list):
        encoded_images = [req.get("image_b64")] if req.get("image_b64") else []
    encoded_images = encoded_images[:4]
    if not encoded_images:
        return {"error": "missing image"}
    try:
        image_bytes = [base64.b64decode(value) for value in encoded_images]
    except Exception:
        return {"error": "bad image"}
    ...
    for index, data in enumerate(image_bytes):
        try:
            source = await deps.keeper_for_acc(acc_id).upload_image(
                data,
                filename=f"reference_{user_id}_{index + 1}.png",
        ...
        if not source or not source.get("mediaId"):
            return {"error": "upload failed"}
        video_project_id = source.pop("_project_id", None) or video_project_id
...
async def generate_video_frames(deps: Any, req: dict) -> dict:
    """Run start/end-frame interpolation and return downloaded MP4 as base64."""
    prompt = str(req.get("prompt") or "").strip()
    if len(prompt) < 3:
        return {"error": "empty prompt"}
    encoded_images = req.get("images_b64")
    ...
    model_id = str(req.get("video_model") or "veo-lite")
    meta = deps.video_model_meta(model_id)
    if not meta or meta.get("family") != "veo":
        return {"error": "bad video model"}
    aspect = str(req.get("aspect_ratio") or "portrait")
    if aspect not in {"portrait", "landscape"}:
        aspect = "portrait"
    ...
    for index, data in enumerate(image_bytes):
        try:
            source = await deps.keeper_for_acc(acc_id).upload_image(
                data,
                filename=f"frame_{user_id}_{index + 1}.png",
                project_id=video_project_id,
            )
        except Exception:
            deps.log.exception("backend frame upload failed (account %s)", acc_id)
            return {"error": "upload failed"}
        if not source or not source.get("mediaId"):
            return {"error": "upload failed"}
        video_project_id = source.pop("_project_id", None) or video_project_id
    ...
    try:
        ...
        deps.log.exception("backend frames download failed (account %s)", acc_id)
        return {"error": "download failed"}
    if not video_bytes:
        return {"error": "download failed"}
    deps.account_pool.mark_success(acc_id)
