...
- Telegram работает через deep link в `@photozhab_bot` и автоматическое
  подтверждение входа без ручного кода; дополнительных OAuth-секретов не
  требуется. `BOT_USERNAME` должен указывать на этого бота.
