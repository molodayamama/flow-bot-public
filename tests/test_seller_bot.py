...
class SellerMenuTests(unittest.TestCase):
    ...
    def test_seller_service_templates_are_wired_for_separate_env(self) -> None:
        ...
        self.assertIn("Skipping $unit (required env file is missing)", deploy_script)
        self.assertIn('restart_if_installed geminifree-seller-bot "$SELLER_ENV_FILE"', deploy_script)
        ...
        self.assertIn("CREDITS_SQLITE=1", seller_env)
        self.assertIn("METRICS_DB=metrics_seller.db", seller_env)
